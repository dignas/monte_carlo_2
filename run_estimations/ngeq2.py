...
def run_n_geq_2(n: int):
	...
	for i, R in enumerate(p.Rs):
		results_cmc = run_cmc(__no_tests, n, R)
		variance_cmc[i] = variance_estimator(results_cmc)

		stratified_R_proportional = np.full(p.m, np.ceil(R / p.m), dtype=np.int32)
		results_stratified_proportional, sigma_strat = run_stratified(__no_tests, n, stratified_R_proportional, p.m)
		...
		results_stratified_optimal, _ = run_stratified(__no_tests, n, stratified_R_optimal, p.m)
